...
func (r *queryResolver) Payments(ctx context.Context, page *int, size *int, sort *string, order *string) (*graphmodel.PaymentsResponse, error) {
	...
	paginated, err := r.PaymentService.GetPayments(ctx, &model.PaymentsRequest{
		Page:  requestPage,
		Size:  requestSize,
		Sort:  requestSort,
		Order: requestOrder,
	...
	graphPayments := make([]*graphmodel.PaymentResponse, len(*paginated.Data))
	for i, payment := range *paginated.Data {
		graphPayments[i] = &graphmodel.PaymentResponse{
			ID:            int(payment.ID),
	...
	return &graphmodel.PaymentsResponse{
		Data:   graphPayments,
		Paging: (*graphmodel.PageMetadata)(paginated.Paging),
	}, nil
}
...
func (r *queryResolver) SearchPayments(ctx context.Context, id *int, orderID *int, amount *float64, status *string, page *int, size *int, sort *string, order *string) (*graphmodel.PaymentsResponse, error) {
	...
	paginated, err := r.PaymentService.SearchPayments(ctx, &model.PaymentSearchRequest{
		ID:      idUint,
		OrderID: orderIDUint,
		Amount:  amountFloat,
		Status:  statusStr,
